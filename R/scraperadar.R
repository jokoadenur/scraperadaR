...
scraperadar <- function(wilayahradar, x) {
  ...
  if (wilayahradar == "kediri") {
    ...
    radartuban <- data.frame()
    for(i in seq(1, 3, 1)){
      url <- paste0("https://radartuban.jawapos.com/daerah?page=",i)
      laman <- read_html(url)
      judul <- laman %>% html_nodes('.latest__link') %>% html_text()
      tglberita <- laman %>% html_nodes('.latest__date') %>% html_text()
      link_judul <- laman %>% html_nodes('.latest__link') %>% html_attr("href")
    ...
    radartuban$lokasi <- rep("Tuban", nrow(radartuban))
    ...
    View(radartuban)
    write.xlsx(radartuban, paste0("radartuban_", Sys.Date(),".xlsx"))
    return(radartuban)
  } else {
    cat("Maaf, masih terbatas pada wilayah tuban, kediri, surabaya, madura, tulungagung, dan semarang\n")
  }
}
